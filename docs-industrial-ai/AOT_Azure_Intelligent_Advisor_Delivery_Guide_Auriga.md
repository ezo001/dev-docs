---
sidebar_position: 2
title: AOT Azure Intelligent Advisor Delivery Guide Auriga
---
...
[Leveraging Azure [5](#leveraging-azure)](#leveraging-azure)
...
[KPI Data Permissions [18](#kpi-data-permissions)](#kpi-data-permissions)
...
-   rajnish.kumar.singh@accenture.com

-   reymark.q.bacalso@accenture.com
...
## Glossary
  ...
  **Term**                           **Definition**
  ---------------------------------- ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
  AOT (Accenture Operations Twin)    A collection of software accelerators and tools, including Intelligent Advisor, that integrate product, process, and live data from IT and OT systems to provide a comprehensive view of operations for better decision-making.

  Intelligent Advisor (IA)           An AI-based solution within AOT that generates, prioritizes, and contextualizes insights and recommendations for users, focusing on critical issues using real-time data.
  ...
  Insight Factory                    The system or process for generating, categorizing, and managing insights within the AOT solution.citeturn1search1
  ...
  Azure SQL Database                 The cloud-based database platform used to store insight categories, configurations, templates, and scheduler data for the AOT solution.
...
1.  Work with an AOT Business Analyst (BA) to create an [Excel file to define the insight categories](https://digitalplantproject.visualstudio.com/Marilyn%20V/_git/AOT-Azure?path=/Processing/IntelligentAdvisor/ia_middleware/ia_func_ms/app/resources/Insight_Factory_Category.xlsx&version=GBdevelop&_a=contents) that are going to be created. This file will be uploaded to the blob storage by running the pipeline (AOT-Azure-IAFunction-ExcelFileUploadPipeline).
...
-   Excel file upload Pipeline: *AOT-Azure-IAFunction-ExcelFileUploadPipeline*

-   IA Function Pipeline: *AOT-Azure-IA-Function-MS*
...
![Automatic Insight Generation Diagram](media/image2.jpeg)\{width="4.71875in" height="9.498134295713037in"\}
...
This microservice does not allow the creation of Insights based on the configuration of the KPI with existing KPIs in Azure DB, as well as for Predictive configuration algorithms with existing assets.
...
![Diagram that depicts the Generic Scheduler and CRUD operations (Create, Read, Update, Delete) performed on tasks.](media/image3.png)\{width="6.895255905511811in" height="2.594034339457568in"\}As shown in the previous diagram, both the AOT front end and the Postman client are used to interact with the scheduler. All of the components in the diagram are briefly explained in the table that follows.
...
The Model Microservice APIs are described in the following table.
  ...
  **API**                    **Description**
  ...
  ModelTrigger API           This API triggers the model to start consuming the messages and perform the operations.
  -------------------------------------------------------------------------------------------------------------------------------------------------------

Note that the ModelTrigger API is triggered when the AOT-Azure-IA-MW-MS Middleware microservice is run and the getInsightList API is hit for the first time.
...
![table structure of sql-ia-config-dev-dev](media/image5.png)\{width="3.8594313210848643in" height="2.910819116360455in"\}
...
![DB structure in azure](media/image8.png)\{width="3.200948162729659in" height="3.464315398075241in"\}
...
2.  Upload the Excel file to the blob storage by running the pipeline (AOT-Azure-IAFunction-ExcelFileUploadPipeline).
...
> ![IA configuration of the insight template in SQL table](media/image12.png)\{width="6.84257217847769in" height="2.174900481189851in"\}

The template is used by the UI and has details about the insights.

![Insight details visible on the UI](media/image13.png)\{width="6.5in" height="2.87798665791776in"\}
