---
sidebar_position: 2
title: AOT Smart KPIs PM Integration Guide Auriga
---
...
[Smart KPI -- People Management Integration [5](#smart-kpi-people-management-integration)](#smart-kpi-people-management-integration)

[Data Permission Microservice [5](#data-permission-microservice)](#data-permission-microservice)

[Workflow [5](#workflow)](#workflow)

[Workflow Diagram [6](#workflow-diagram)](#workflow-diagram)

[Viewing KPIs [7](#viewing-kpis)](#viewing-kpis)

[APIs [9](#apis)](#apis)

[Authentication Token [9](#authentication-token)](#authentication-token)

[People Management Role Retrieval API [9](#people-management-role-retrieval-api)](#people-management-role-retrieval-api)

[People Management Role Detail API [10](#people-management-role-detail-api)](#people-management-role-detail-api)

[Adding Roles to Assets [11](#adding-roles-to-assets)](#adding-roles-to-assets)

[Sample Postman Query [11](#sample-postman-query)](#sample-postman-query)

[Adding Roles to Timeseries [12](#adding-roles-to-timeseries)](#adding-roles-to-timeseries)

[Sample Postman Query [12](#sample-postman-query-1)](#sample-postman-query-1)

[Sample Python Script [12](#sample-python-script)](#sample-python-script)

[Use Cases [13](#use-cases)](#use-cases)

[Use Case 1: GET- KPI dashboard [13](#use-case-1-get--kpi-dashboard)](#use-case-1-get--kpi-dashboard)

[Use Case 2: GET -- List API [13](#use-case-2-get-list-api)](#use-case-2-get-list-api)

[Use Case 3: POST- KPI Drilldown [14](#use-case-3-post--kpi-drilldown)](#use-case-3-post--kpi-drilldown)
...
-   This guide describes how to integrate the people management component with other AOT components and describes PM and Smart KPI integration as an example.
...
-   [AOT Release Notes](https://industryxdevhub.accenture.com/assetdetails/45)[Cognite API Docs](https://apim-aot-mw-dev.azure-api.net/api/people-management/roles/%7broleId)
...
## Glossary
  ...
  Term                               Definition
  ---------------------------------- --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
  AOT (Accenture Operations Twin)    A suite of software accelerators and tools that integrate product, process, and live data from various IT and OT systems, providing a comprehensive view of operations for improved decision-making.

  Smart KPIs                         A micro-frontend application within AOT that displays contextualized Key Performance Indicators (KPIs) to users, enabling performance tracking and improvement actions. Main landing page of AOT.

  People Management (PM)             A component of AOT that allows administrators to manage user roles and access to data and platform functionality. Integrates with Smart KPIs to filter data by user roles.

  KPI (Key Performance Indicator)    A measurable value that demonstrates how effectively an organization is achieving key business objectives. In AOT, KPIs are tracked and managed through Smart KPIs.

  Asset Hierarchy (AH)               The structured organization of assets within AOT, which determines how roles and permissions are assigned and how users can access KPIs.

  Timeseries                         A sequence of data points indexed in time order, used in AOT to represent KPIs and other business metrics.

  Role                               A set of permissions assigned to users or groups, defining their access to data and functionality within AOT and Smart KPIs. Roles can be mapped directly to users or indirectly via Active Directory (AD) groups.

  Active Directory (AD) Groups       Groups managed in Azure Active Directory, used to assign roles and permissions to users within AOT.

  Authentication Token               A security token required to access People Management APIs, typically acquired via Azure AD.

  Event Broker / Message Broker      A system that stores and distributes events (such as role changes) to subscribed microservices, ensuring that permissions and roles are updated across AOT components.

  Microservice                       A small, independent service that performs a specific function within the AOT architecture, such as managing roles or updating permissions.
  ...
  Postman                            An API testing tool used to manually update roles and permissions in AOT assets and timeseries.
  -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
...
-   The purpose of the Data Permission Microservice is to align the roles of AOT users with the roles defined in the configuration template. The roles are updated in Timeseries metadata on CDF.
...
![data permission workflow diagram](media/image2.png)\{width="7.140365266841645in" height="2.3020833333333335in"\}
...
![AOT application interface showing the access assigned to user](media/image3.png)\{width="6.5in" height="2.716666666666667in"\}

![AOT application interface showing the performance KPI](media/image4.png)\{width="6.5in" height="2.7083333333333335in"\}
...
![Screenshot of performance plant](media/image5.png)\{width="6.4072517497812775in" height="2.84632217847769in"\}
...
![Screenshot showing the time series tab with values of Role IDs highlighted.](media/image6.png)\{width="6.398303805774278in" height="3.463076334208224in"\}
...
#### Specifications
  ...
  PATH (Public Exposure)(AWS)   https://cn13sbr3r6.execute-api.us-west-2.amazonaws.com/api/people-management/roles/byIds
...
![Screenshot showing Postman query to add user roles to assets](media/image7.png)\{width="6.776750874890639in" height="2.5478707349081366in"\}
...
![Screenshot of assets visible in CDF admin portal](media/image8.png)\{width="6.746662292213474in" height="3.6047648731408573in"\}
...
![Screenshot of queries to add roles to timeseries](media/image9.png)\{width="6.766874453193351in" height="2.6857819335083115in"\}
...
![Screenshot of the roles visible in Timeseries ](media/image10.png)\{width="6.5in" height="3.9115102799650043in"\}
...
The KPI drilldown API is part of the AOT-SmartKPI-Middleware Microservice that is invoked and consumed by the UI. It is a POST call to the Smart KPI AOT server hosted at the backend to get the contributing and influencing KPI details (external IDs) which are displayed to the user on clicking on the KPI tile. The contributing and influencing KPIs are defined in the following table:

  --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
  Contributing KPIs                                                                                                                                                                                                                                                       Influencing KPIs
  ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
  This represents all the KPIs of a selected KPI that directly contribute to the value/performance of the selected KPI. For example: for OEE at the plant level - the contributing KPIs can be availability, performance, and OEE at the system level among other KPIs.   This represents all the KPIs of a selected KPI that indirectly influence the value/performance of the selected KPI. For example: for OEE at the plant level - the influencing KPIs can be the number of safety incidents among other KPIs.

  --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------

The drilldown details, i.e., the contributing and influencing KPI details, displayed to a user are based on the user\'s role authorizations, which the Drilldown API fetches from a People Management API, GetAOTRoleById.

Along with the user role, sensitivity tags of the KPI are used by the API to decide whether a user can view that KPI in drilldown or not. The POST KPI DRILLDOWN API fetches the sensitivity tag of the user\'s role from the PM API to validate against and display the drilldown details accordingly. The following points explain how a user\'s role and permissions affect the drilldown view.
