---
sidebar_position: 2
title: AOT KPI Hierarchy and Calculation Technical Overview Azure Auriga
---
...
[KPI Hierarchies [5](#kpi-hierarchies)](#kpi-hierarchies)

[Hierarchy Flows [7](#hierarchy-flows)](#hierarchy-flows)

[Create KPI Hierarchy Function [8](#create-kpi-hierarchy-function)](#create-kpi-hierarchy-function)

[UID/Config ID Generation [9](#uidconfig-id-generation)](#uidconfig-id-generation)

[KPI Hierarchy [9](#kpi-hierarchy)](#kpi-hierarchy)

[Configuration Module [9](#configuration-module)](#configuration-module)

[Update Status and Version [9](#update-status-and-version)](#update-status-and-version)

[KPI Calculations [10](#kpi-calculations)](#kpi-calculations)

[Scheduling and Sequencing [10](#scheduling-and-sequencing)](#scheduling-and-sequencing)

[Orchestrator Microservice [11](#orchestrator-microservice)](#orchestrator-microservice)

[Simulated Value Creation [14](#simulated-value-creation)](#simulated-value-creation)

[Execution of Scheduled Functions [15](#execution-of-scheduled-functions)](#execution-of-scheduled-functions)

[UoM Configuration [16](#uom-configuration)](#uom-configuration)

[UoM APIs [16](#uom-apis)](#uom-apis)

[UoM Conversion Formula [16](#uom-conversion-formula)](#uom-conversion-formula)

[Conversion Procedure [17](#conversion-procedure)](#conversion-procedure)

[Computation Engine [19](#computation-engine)](#computation-engine)

[Workflow of the Calculation Process [20](#workflow-of-the-calculation-process)](#workflow-of-the-calculation-process)

[Hierarchical Calculation Breakdown [20](#hierarchical-calculation-breakdown)](#hierarchical-calculation-breakdown)

[Key Functional Components [20](#key-functional-components)](#key-functional-components)

[Calculation Flow for KPI Hierarchy [21](#calculation-flow-for-kpi-hierarchy)](#calculation-flow-for-kpi-hierarchy)

[Orchestrator Tables [21](#orchestrator-tables)](#orchestrator-tables)

[POST Web PubSub API [21](#post-web-pubsub-api)](#post-web-pubsub-api)
...
Key Performance Indicators (KPIs) are measurable factors that are tracked to measure performance, stimulate actions, and drive business productivity. The factors listed in the table below are used to form each KPI.
  ...
  **Factor**             **Description**
  ...
  Forecast               Forecast for the current time interval, which is calculated as a 7-day moving average.
  ---------------------------------------------------------------------------------------------------------------------------------------

AOT is integrated with Azure Resources where Schedulers are used to calculate the KPI data using the factors above and to insert data points into the timeseries. For the scheduler to work, a KPI Hierarchy must first be established.
...
![KPI hierarchy in AOT](media/image2.png)\{width="13.301434820647419in" height="5.711303587051619in"\}

The following image depicts the KPI details in the Azure Digital Twins (ADT) Explorer.

![ KPI details in the Azure Digital Twins (ADT) Explorer.](media/image3.png)\{width="14.0in" height="6.616666666666666in"\}
...
The below diagrams represent asset hierarchy, KPI hierarchy, and parameters in ADT.
  ...
  ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
  ![asset hierarchy, KPI hierarchy and parameters in ADT.](media/image4.png)\{width="6.360416666666667in" height="7.149305555555555in"\}   ![asset hierarchy, KPI hierarchy and parameters in ADT.](media/image5.png)\{width="6.348837489063867in" height="7.944937664041995in"\}
  ------------------------------------------------------------------------------------------------------------------------------------------- -------------------------------------------------------------------------------------------------------------------------------------------
...
Asset hierarchy creation will happen by Azure Data Factory pipelines after uploading the asset hierarchy template in blob storage. Asset Hierarchy nodes are represented by blue nodes where the root node is C \"company\", which has a child C-B1 \"Line of Business\". Both have the relationship \"contains\", the direction of which is from parent to child. Similarly, each node can have single or multiple children till the leaf node (leaf nodes do not have any children).

Asset Hierarchy is divided into two parts Multi-plant asset hierarchy and Plant asset hierarchy. Nodes above plant level \"C-B1-G1-R1-F1\" belong to Multi-plant asset hierarchy nodes. Plant level and below plant level come under the Plant asset hierarchy. There can be multiple plant asset hierarchies, and thus the hierarchy above the plant level is referred to as a Multi-plant asset hierarchy. Each asset node from the asset hierarchy can have zero or more KPIs with the relationship \"KPI_For\" and they also have zero or more parameters with the relationship \"Parameter_for.\"

**\
KPI Hierarchy**

KPI hierarchy creation occurs after uploading the KPI hierarchy template in the SmartKPI microservice. KPI Hierarchy nodes are represented by orange nodes. These nodes either have a contributor or influencer relation where the contributor is directly influencing the KPI, and the influencer is indirectly influencing the KPI whose direction is from lower-level KPIs to Upper-level KPIs (as lower-level KPIs directly or indirectly influence the upper-level KPIs). The level of a KPI is determined by the level of the asset with which the KPI has a \"KPI_For\" relationship.

KPIs can also have parameters as their contributors or influencers. A KPI can have more than one contributing/influencing KPI or more than one contributing/influencing parameter but one KPI can only be associated with one asset. Metadata information of a KPI can be configured in the KPI hierarchy template and the same information appears in ADT.

**\
Parameter Hierarchy**

Parameters are created by the simulator. Parameter Hierarchy nodes are represented by green nodes. Parameter nodes are the leaf nodes, and thus they do not have any contributor or influencing KPIs or parameters. A parameter can have the relationship \"Parameter_for\" with only one asset, but an asset can have zero or more parameters. Parameter metadata is inserted by the simulator.

Using a query, any type of node can be retrieved in the Azure Digital Twin Explorer. Clicking on the node fetches the metadata of the respective Asset, KPI, or parameter.
...
![the order of flow from one component to another](media/image6.png)\{width="6.650666010498687in" height="2.5584416010498687in"\}
...
![scheduling and sequencing process flow diagram](media/image7.png)\{width="6.837209098862642in" height="3.321621828521435in"\}
...
![the architecture of the orchestrator service](media/image8.png)\{width="10.801101268591426in" height="5.868405511811024in"\}
...
|  | \"Status\": \"Initiated\", |
|  | \"Level\": \"System\", |
|  | \"kpiname\": \"Actual Productivity\" |
|  | \}\] |
|  | \} |
| **STARTED** | \{ |
| The following code depicts how the Orchestrator Service receives Kafka Messages from the Computation Engine when the status equals *STARTED*. After receiving the message, the document (record/file) in the Mongo DB is updated with the status *ONGOING*. | \"Type\": \"Calculation Event\", |
|  | \"Scopes\": \[\"Smart KPI Orchestration\"\], |
|  | \"Timestamp\": \"2023-08-24 09:31:16\", |
|  | \"Version\": \"1\", |
|  | \"Payload\": \[\{ |
...
|  | \"Status\": \"Completed\", |
|  | \"Level\": \"System\", |
|  | \"kpiname\": \"Actual Prodcutivity\", |
|  | \"actual_starttime\": \"2023-08-24 09:20:00\", |
|  | \"actual_endtime\": \"2023-08-24 09:30:00\", |
|  | \"execution_endtime\": \"2023-08-24 09:31:16\" |
|  | \}\]\} |
...
![Simulated value creation](media/image9.png)\{width="14.536687445319336in" height="6.589820647419073in"\}
...
The APIs used in the UoM implementation are as follows:
  ...
  3    GetUoMConversions      This API fetches all the conversion a, b, c,- d values irrespective of unit systems.
  -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------

For more information on UoM APIs, refer to the [Units of Measurement Technical Reference](https://industryxdevhub.accenture.com/assetdetails/87).

###  UoM Conversion Formula

The UoM conversion is done by the following formula:
...
-   a, b, c, and d= pre-defined values to convert from source UoM to target.

The Actual calculation formula consists of all contributors: child1, child2, child3
...
Units of measurement**
  ...
  **Unit**        **SI (primary)**                        **Field**
  ...
  MT3             UoM3, UoM3.1 (primary)                  \-
  --------------------------------------------------------------------------

**Flow**

![UoM Conversion procedure flow diagram ](media/image10.png)\{width="13.8222375328084in" height="7.71875in"\}
...
-   For Batch KPIs, the system uses event-based data from Cosmos DB to determine time ranges and compute KPIs using the corresponding time-series data.\
    ![Diagram showing calculation of actual value at plant level](media/image11.png)\{width="7.608758748906387in" height="3.3854166666666665in"\}
...
![details of the KPI ](media/image12.png)\{width="6.344903762029746in" height="2.8020833333333335in"\}
