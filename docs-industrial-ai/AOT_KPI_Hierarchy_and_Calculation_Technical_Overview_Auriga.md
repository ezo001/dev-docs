---
sidebar_position: 2
title: AOT KPI Hierarchy and Calculation Technical Overview Auriga
---
...
[KPI Hierarchies [4](#glossary)](#glossary)

[Create KPI Hierarchy Function [8](#create-kpi-hierarchy-function)](#create-kpi-hierarchy-function)

[UID/Config ID Generation [8](#uidconfig-id-generation)](#uidconfig-id-generation)

[Create JSON [8](#create-json)](#create-json)
...
[Scheduling [10](#scheduling)](#scheduling)

[Orchestrator Microservice [12](#orchestrator-microservice)](#orchestrator-microservice)

[Kafka Messages [13](#kafka-messages)](#kafka-messages)

[Table Details [15](#table-details)](#table-details)

[API Specifications [15](#api-specifications)](#api-specifications)

[Simulated Value Creation [16](#simulated-value-creation)](#simulated-value-creation)

[Execution of Scheduled Functions [17](#execution-of-scheduled-functions)](#execution-of-scheduled-functions)

[Computation of Actual Based on KPI Formula [17](#computation-of-actual-based-on-kpi-formula)](#computation-of-actual-based-on-kpi-formula)

[UoM Configuration [18](#uom-configuration)](#uom-configuration)

[Computation Engine [21](#computation-engine)](#computation-engine)

[API Specifications [22](#api-specifications-1)](#api-specifications-1)
...
AOT is integrated with the Cognite Data Fusion (CDF) DataOps platform where the Computation engine is used to calculate the KPI data using the factors above and to insert data points into the timeseries. For the Computation engine to work, a KPI Hierarchy must first be established.
...
## Glossary
  ...
  ----------------------------------------- -----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
  AOT (Accenture Operations Twin)           A collection of software accelerators and tools designed to integrate product, process, and live data from disparate IT and OT systems, providing a comprehensive view of operations for better decision-making and optimization.
  ...
  CDF (Cognite Data Fusion)                 A DataOps platform integrated with AOT, used for storing and processing KPI data, including timeseries data and computation logic.

  KPI Hierarchy                             The structured organization of KPIs, including relationships such as contributing, influencing, and parent-child status, used to manage assets and metadata in AOT.
...
![KPI Hierarchy in CDF](media/image2.png)\{width="11.12438210848644in" height="6.286111111111111in"\}

![KPI Hierarchies](media/image3.png)\{width="12.520833333333334in" height="5.967361111111111in"\}

The following image depicts the KPI metadata along with the Config ID in the KPI Timeseries.

![KPI metadata with config ID in the KPI timeseries](media/image4.png)\{width="13.281492782152231in" height="6.615972222222222in"\}
...
![Create KPI Hierarchy function flow diagram](media/image5.png)\{width="7.972328302712161in" height="2.3488363954505687in"\}
...
![KPI calculation example](media/image6.png)\{width="3.577298775153106in" height="1.6596719160104987in"\}
...
See the [AOT Generic Scheduler Technical Overview](https://industryxdevhub.accenture.com/assetdetails/83) for information about scheduling expressions.
...
![scheduling database table](media/image7.png)\{width="8.741071741032371in" height="4.268210848643919in"\}

![scheduling database table](media/image8.png)\{width="8.776785870516186in" height="3.375797244094488in"\}

## Orchestrator Microservice

The following diagram depicts the architecture of the orchestrator service in further detail.

![Orchestrator Microservice architecture diagram](media/image9.png)\{width="9.813186789151356in" height="4.086805555555555in"\}
...
![Screenshot of CDF showing how the simulated values are stored in timeseries](media/image10.png)\{width="14.115343394575678in" height="4.014050743657043in"\}
...
For more information on UoM APIs, refer to the [Units of Measurement API Reference](https://industryxdevhub.accenture.com/assetdetails/87).
...
10. The parent KPI data is converted from local UoM to standard Primary UoM before inserting into CDF.
...
The following flow diagram illustrates the conversion procedure for an example scenario with the following details:

Plant 1 has the following KPI details
...
![UoM Conversion procedure flow diagram](media/image11.png)\{width="11.1317760279965in" height="6.202380796150481in"\}
...
![Diagram showing calculation of actual value at plant level](media/image12.png)\{width="6.216036745406824in" height="2.8604647856517937in"\}
