---
sidebar_position: 2
title: AOT Flat File Extractor Deployment Guide
---
...
| **Related Assets / Alternatives** | AOT Extractors Architecture Blueprint, AOT Extractors Getting Started ![](./media/AOT_Flat_File_Extractor_Deployment_Guide/image2.png)
|  |
...
> ![Screenshot of a create a secret window](./media/AOT_Flat_File_Extractor_Deployment_Guide/image3.png)
>
> ![image](./media/AOT_Flat_File_Extractor_Deployment_Guide/image4.png)
3.  Create a key vault library in Azure DevOps and add the secrets created in the previous step.

> ![Screenshot of the vault library with the secrets added](./media/AOT_Flat_File_Extractor_Deployment_Guide/image5.png)
4.  Create a Library/Variable Group in Azure DevOps with the following parameters:

> ![screenshot of the Variable group showing the project key, project name, and the config file name](./media/AOT_Flat_File_Extractor_Deployment_Guide/image6.png)
...
| 5. | Navigate to the pipeline file location \"Source/Extractors/FlatFile-Extractor/pipeline/Dev/azure-pipelines.yml\" and ensure that the pipeline is referring to the libraries created in the previous steps. &gt; ![screenshot of the pipeline file showing the libraries created in previous steps](./media/AOT_Flat_File_Extractor_Deployment_Guide/image7.png)
|  |
| 6. | Configure the following parameters in the configuration file for the Flat File Extractor: |
| A. | Provide the timer value in seconds to schedule the extractor. &gt; ![Screenshot of the timer-value provided to schedule the extractor](./media/AOT_Flat_File_Extractor_Deployment_Guide/image8.png)
|  |
| B. | Provide mandatory Cognite-related config parameters: &gt; ![Screenshot of the parameters specified to connect to Cognite server](./media/AOT_Flat_File_Extractor_Deployment_Guide/image9.png)
|  |
...
| contacts.sendnotification | Provide notification value. If the send notification value is true, then mail is sent to the respective mail Id else no mail is sent. O &gt; ![Screenshot of extraction pipeline related config parameters](./media/AOT_Flat_File_Extractor_Deployment_Guide/image11.png)
|  |
...
> ![Screenshot of Azure DevOps showing the option \'New Pipeline\'](./media/AOT_Flat_File_Extractor_Deployment_Guide/image12.png)
2.  
...
> ![Screenshot of ADO showing the \'Azure Repos Git\' option](./media/AOT_Flat_File_Extractor_Deployment_Guide/image13.png)
5.  Select \'Existing Azure Pipelines YAML file\'.

    ![Screenshot of Azure DevOps showing the Configure tab and the \'Existing Azure Pipelines YAML file\' option.](./media/AOT_Flat_File_Extractor_Deployment_Guide/image14.png)
...
8.  Review and save.
    ...
    ![Screenshot of Configure tab of ADO\'s pipelines page, showing the dialog that has options to input the file path](./media/AOT_Flat_File_Extractor_Deployment_Guide/image15.png)

9.  Run the pipeline and wait for its successful completion.

> ![Screenshot showing the Run pipeline option](./media/AOT_Flat_File_Extractor_Deployment_Guide/image16.png)
...
> ![Screenshot of the updated value of the service connection for AKS cluster in the tasks](./media/AOT_Flat_File_Extractor_Deployment_Guide/image17.png)
3.  

4.  Update the AKS file location in the tasks.

> ![Screenshot of the updated AKS file location in the tasks](./media/AOT_Flat_File_Extractor_Deployment_Guide/image18.png)
5.  If this is the first run, then disable the delete task from the pipeline.

> ![Screenshot showing the option to enable and disable the selected task](./media/AOT_Flat_File_Extractor_Deployment_Guide/image19.png)
6.  Create a release from the release pipeline created in the previous step.

> ![Screenshot showing the Create release option](./media/AOT_Flat_File_Extractor_Deployment_Guide/image20.png)
7.  Once the release is completed, confirm successful deployment on Azure DevOps.

> ![Screenshot of the successful deployment on Azure DevOps](./media/AOT_Flat_File_Extractor_Deployment_Guide/image21.png)
8.  After deployment is complete, validate successful deployment to the AKS cluster in the Azure portal.

![Screenshot of successful deployment to AKS cluster in Azure portal](./media/AOT_Flat_File_Extractor_Deployment_Guide/image22.png)

9.  Validate Flat File Extractor logs on the [Lens](https://k8slens.dev/) app.

![Screenshot of Lens app where the logs of AKS cluster are viewed and validated](./media/AOT_Flat_File_Extractor_Deployment_Guide/image23.png)

10. If everything looks good and the AKS POD is created, enable the delete task that was previously disabled.

> ![Screenshot of enable option of the delete task that was previously disabled.](./media/AOT_Flat_File_Extractor_Deployment_Guide/image24.png)
11. 

12. Validate the creation of the database and the table with asset hierarchy data on the CDF portal.

> ![Screenshot of CDF showing the created database and tables with asset hierarchy data](./media/AOT_Flat_File_Extractor_Deployment_Guide/image25.png)
...
> ![Screenshot of the Success message in CDF. The message reads: Extraction pipeline ran successfully for 1 sec and uploaded 206 rows. The execution start and end time is also specified.](./media/AOT_Flat_File_Extractor_Deployment_Guide/image26.png)
![Screenshot of the Failure message in CDF. The message reads: \"Extraction Pipeline failed while making batch request to PI server occurred at post_batch_request function step.\" Execution start and end time is specified and message continues: \"Reason for failure exception occurred while making batch request to PI server\".](./media/AOT_Flat_File_Extractor_Deployment_Guide/image27.png)
...
> ![Screenshot of the log file path to store extractor logs](./media/AOT_Flat_File_Extractor_Deployment_Guide/image28.png)
B.  Add timer value in seconds to schedule the extractor.

> ![Screenshot of the timer_value in seconds that is provided to schedule the extractor](./media/AOT_Flat_File_Extractor_Deployment_Guide/image29.png)
...
| G. | If the target CSV files are stored locally, then provide the following details as well. &gt; ![Screenshot of local storage related details](./media/AOT_Flat_File_Extractor_Deployment_Guide/image30.png)
|  |
...
| - | pythonXX.dll |
| - | pythoncomXX.dll |
| - | pywintypesXX.dll &gt; ![Screenshot showing the files copied to respective location.](./media/AOT_Flat_File_Extractor_Deployment_Guide/image32.png)
|  |
...
> ![Screenshot of the Start menu showing the Services App and the run as administrator option.](./media/AOT_Flat_File_Extractor_Deployment_Guide/image33.png)
2.  Search for \"FlatFileExtractor\" service name, right-click on it, and select *Start*.

> ![Screenshot to show how to search for "FlatFileExtractor" and select "Start".](./media/AOT_Flat_File_Extractor_Deployment_Guide/image34.png)
...
![Screenshot of CDF, where the created database and tables are validated](./media/AOT_Flat_File_Extractor_Deployment_Guide/image25.png)

5.  Validate the creation of the extraction pipeline with the success message on the CDF portal.

![Screenshot of the success message in CDF portal](./media/AOT_Flat_File_Extractor_Deployment_Guide/image26.png)

### Stop the Extractor

1.  Click the *Start* button, search for \"Services\", and select *Run as Administrator.*

> ![Screenshot of Windows Start menu showing the Services App and the run as administrator option](./media/AOT_Flat_File_Extractor_Deployment_Guide/image33.png)
2.  Search for \"FlatFileExtractor\" service name, right-click on it, and select *Stop*.

> ![Screenshot of the services app where the option to Stop the Flat File Extractor service is shown](./media/AOT_Flat_File_Extractor_Deployment_Guide/image35.png)
