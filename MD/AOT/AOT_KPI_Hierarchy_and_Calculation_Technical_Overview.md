---
sidebar_position: 2
title: AOT KPI Hierarchy and Calculation Technical Overview
---
...
| Forecast | Forecast for the current time interval, which is calculated as a 7-day moving average. AOT is integrated with the Cognite Data Fusion (CDF) DataOps platform where the Computation engine is used to calculate the KPI data using the factors above and to insert data points into the timeseries. For the Computation engine to work, a KPI Hierarchy must first be established. |
...
![KPI Hierarchy in CDF](./media/AOT_KPI_Hierarchy_and_Calculation_Technical_Overview/image2.png)

![KPI Hierarchies](./media/AOT_KPI_Hierarchy_and_Calculation_Technical_Overview/image3.png)

The following image depicts the KPI metadata along with the Config ID in the KPI Timeseries.

![KPI metadata with config ID in the KPI timeseries](./media/AOT_KPI_Hierarchy_and_Calculation_Technical_Overview/image4.png)
...
![Create KPI Hierarchy function flow diagram](./media/AOT_KPI_Hierarchy_and_Calculation_Technical_Overview/image5.png)
...
![KPI calculation example](./media/AOT_KPI_Hierarchy_and_Calculation_Technical_Overview/image6.png)
...
![scheduling database table](./media/AOT_KPI_Hierarchy_and_Calculation_Technical_Overview/image7.png)

![scheduling database table](./media/AOT_KPI_Hierarchy_and_Calculation_Technical_Overview/image8.png)

### Orchestrator Microservice

The following diagram depicts the architecture of the orchestrator service in further detail.

![Orchestrator Microservice architecture diagram](./media/AOT_KPI_Hierarchy_and_Calculation_Technical_Overview/image9.png)
...
![Screenshot of CDF showing how the simulated values are stored in timeseries](./media/AOT_KPI_Hierarchy_and_Calculation_Technical_Overview/image10.png)
...
| - | y= target UoM data |
| - | x=source UoM data, |
| - | a, b, c, and d= pre-defined values to convert from source UoM to target. The Actual calculation formula consists of all contributors: child1, child2, child3 **Conversion Procedure** The following steps describe the process of converting the standard AOT UoM to the user-configured (local) UoM. |
...
| **MT2** | UoM2 (primary) \- |
| **MT3** | UoM3, UoM3.1 (primary) \- ![UoM Conversion procedure flow diagram](./media/AOT_KPI_Hierarchy_and_Calculation_Technical_Overview/image11.png)
|  |
...
![Diagram showing calculation of actual value at plant level](./media/AOT_KPI_Hierarchy_and_Calculation_Technical_Overview/image12.png)
