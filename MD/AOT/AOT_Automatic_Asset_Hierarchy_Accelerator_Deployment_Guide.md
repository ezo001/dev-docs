---
sidebar_position: 2
title: AOT Automatic Asset Hierarchy Accelerator Deployment Guide
---
...
> ![Screenshot showing the \'Create a secret\' window and fields to be filled.](./media/AOT_Automatic_Asset_Hierarchy_Accelerator_Deployment_Guide/image2.png)
-   Create a key vault library in Azure DevOps and add the secrets created in the previous step.

> ![Screenshot showing the variable group and the secrets added to it.](./media/AOT_Automatic_Asset_Hierarchy_Accelerator_Deployment_Guide/image3.png)
...
| AUTOMATIC_AH_CONFIG_FILENAME | Provide the configuration filename that is used to run the extractor. The filenames are environment-specific for assethierarchy_dev_config.YAML, assethierarchy_itest_config.YAML and assethierarchy_prod_config.YAML &gt; ![Screenshot of Variable group tab showing the parameters](./media/AOT_Automatic_Asset_Hierarchy_Accelerator_Deployment_Guide/image4.png)
|  |
| - | Navigate to the pipeline file location \"Source/Extractors/AutomaticAssetHierarchy/pipeline/Dev/azure-pipelines.yml\" and ensure that the pipeline refers to the libraries created in the previous steps. &gt; ![Screenshot showing the location of azure-pipelines.yml file.](./media/AOT_Automatic_Asset_Hierarchy_Accelerator_Deployment_Guide/image5.png)
|  |
...
| idp-authentication. token_url | Use the token_url for CDF added in the Azure DevOps library in earlier steps ![screenshot of Cognite parameters](./media/AOT_Automatic_Asset_Hierarchy_Accelerator_Deployment_Guide/image6.png)
|  |
...
| - | CDF offers a variety of Destination and Action Types. Each Destination Type has its own set of Action Types. ![Screenshot showing the parameter and available actions for them](./media/AOT_Automatic_Asset_Hierarchy_Accelerator_Deployment_Guide/image7.png)
|  |
...
> ![Screenshot of Azure DevOps showing the pipelines screen and option for \'New Pipeline\'](./media/AOT_Automatic_Asset_Hierarchy_Accelerator_Deployment_Guide/image9.png)
...
> ![Screenshot of ADO showing the option of Azure Repos Git](./media/AOT_Automatic_Asset_Hierarchy_Accelerator_Deployment_Guide/image10.png)
![Screenshot of ADO showing \'Existing Azure Pipelines YAML file\' option](./media/AOT_Automatic_Asset_Hierarchy_Accelerator_Deployment_Guide/image11.png)
...
![Screenshot showing the window and options to select branch and provide pipeline file path](./media/AOT_Automatic_Asset_Hierarchy_Accelerator_Deployment_Guide/image12.png)

5.  Review and save.

6.  Run the pipeline and wait for its successful completion.

![Screenshot showing the option \'Run pipeline\'.](./media/AOT_Automatic_Asset_Hierarchy_Accelerator_Deployment_Guide/image13.png)
...
> ![Screenshot showing the field that needs to be updated with the value of service connection for AKS cluster](./media/AOT_Automatic_Asset_Hierarchy_Accelerator_Deployment_Guide/image14.png)
3.  Update the AKS file location in the tasks.

> ![Screenshot showing the AKS file location](./media/AOT_Automatic_Asset_Hierarchy_Accelerator_Deployment_Guide/image15.png)
4.  

5.  Disable the delete task after running the pipeline the first time.

> ![Screenshot showing the option to disable the delete task from the pipeline](./media/AOT_Automatic_Asset_Hierarchy_Accelerator_Deployment_Guide/image16.png)
6.  Create a release from the release pipeline created in the previous step.

> ![Screenshot showing the page and option to \'Create release\'.](./media/AOT_Automatic_Asset_Hierarchy_Accelerator_Deployment_Guide/image17.png)
7.  Once the release is completed, confirm that the deployment was successful on Azure DevOps.

> ![Screenshot showing the successful deployment on ADO.](./media/AOT_Automatic_Asset_Hierarchy_Accelerator_Deployment_Guide/image18.png)
8.  

9.  After deployment, validate successful deployment to the AKS cluster in the Azure portal.

> ![Screenshot showing the successful deployment to AKS cluster](./media/AOT_Automatic_Asset_Hierarchy_Accelerator_Deployment_Guide/image19.png)
10. Use [Lens](https://k8slens.dev/) to validate Automatic Asset Hierarchy Accelerator logs.

> ![Screenshot of Lens App](./media/AOT_Automatic_Asset_Hierarchy_Accelerator_Deployment_Guide/image20.png)
11. 

12. If everything looks good and AKS POD is created, enable the delete task that was previously disabled.

> ![Screenshot showing the option to enable and disable selected task](./media/AOT_Automatic_Asset_Hierarchy_Accelerator_Deployment_Guide/image21.png)
13. Validate successful transformation run in CDF.

> ![Screenshot showing the successful transformation run in CDF](./media/AOT_Automatic_Asset_Hierarchy_Accelerator_Deployment_Guide/image22.png)
## 
...
![An example of the Success message visible on CDF portal after deployment and extraction pipeline creation](./media/AOT_Automatic_Asset_Hierarchy_Accelerator_Deployment_Guide/image23.png)

![An example of the Failure message visible on CDF portal after deployment and extraction pipeline creation](./media/AOT_Automatic_Asset_Hierarchy_Accelerator_Deployment_Guide/image24.png)
