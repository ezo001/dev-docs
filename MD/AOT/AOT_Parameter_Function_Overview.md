---
sidebar_position: 2
title: AOT Parameter Function Overview
---
...
![Parameter function architecture flow diagram showing data from Simulator to Azure IoT Hub, then Azure Stream Analytics, Azure Function, and Caching Mechanism, with outputs to Azure Digital Twins.](./media/AOT_Parameter_Function_Overview/image2.png)
...
![Azure Digital Twins Explorer showing a query result for model 'dtmi:com:aot:Parameters;2'. The interface includes a twin graph with multiple nodes and a properties panel displaying details like ADXDatabaseName, ADXTable, ClusterURI, and metadata fields.](./media/AOT_Parameter_Function_Overview/image3.png)
...
| WEBSITE_RUN_FROM_PACKAGE | Indicates if the function app is running directly from a deployment package. This can point to a URL containing a .zip file with the app\'s content or set to \"1\" to run from a package in the current storage account. ![Azure portal showing Function App environment variables for 'func-aot-parameter-metadata-dev'. The screen lists variable names like ADXDatabaseName, ADXTable, APPINSIGHTS_INSTRUMENTATIONKEY, AZURE_CLIENT_ID, AZURE_CLIENT_SECRET, and others with masked values.](./media/AOT_Parameter_Function_Overview/image4.png)
|  |
