---
id: aot-file-handler-api-reference-auriga
title: AOT File Handler API Reference Auriga
---
...
| **Field** | **Value** |
|----|----|
| **Asset / Solution Name** | Accenture Operations Twin / File Handler |
...
| **Related Assets / Alternatives** | AOT Getting Started |
...
[API Architecture [4](#api-architecture)](#api-architecture)

[File Handler APIs [4](#file-handler-apis)](#file-handler-apis)

[Operations Hierarchy Configuration APIs [4](#operations-hierarchy-configuration-apis)](#operations-hierarchy-configuration-apis)

[POST-Upload File [5](#post-upload-file)](#post-upload-file)

[Specification [5](#specification)](#specification)

[Input Header [5](#input-header)](#input-header)

[Input Body [5](#input-body)](#input-body)

[Output Header [5](#output-header)](#output-header)

[Output Body [5](#output-body)](#output-body)

[Result [5](#result)](#result)

[Error Management [5](#error-management)](#error-management)

[Sample JSON Response [5](#sample-json-response)](#sample-json-response)

[POST-Download File [6](#post-download-file)](#post-download-file)

[Specification [6](#specification-1)](#specification-1)

[Input Header [6](#input-header-1)](#input-header-1)

[Input Body [6](#input-body-1)](#input-body-1)

[Output Header [6](#output-header-1)](#output-header-1)

[Output Body [6](#output-body-1)](#output-body-1)

[Result [6](#result-1)](#result-1)

[Error Management [6](#error-management-1)](#error-management-1)

[Sample JSON Request [6](#sample-json-request)](#sample-json-request)
...
AOT’s File Handler component facilitates the uploading and downloading of files using Azure blob storage. It is a standalone component that is utilized by other AOT components such as Operation Hierarchy’s Entity Viewer and Smart KPIs to upload and download template files. This component also provides download functionality for the latest templates used for KPI configuration and Entity Viewer configuration based on Plant and Asset Type details.
...
``
``
Application Programming Interface; a set of protocols and tools for building and interacting with software applications.
``
``
``
``
...
``
``
Services that enable the creation, publication, and management of APIs in a secure and scalable manner.
``
``
``
``
...
``
``
Cloud computing services provided by Microsoft Azure, used for hosting, storing, and managing data and applications.
``
``
``
``
...
``
``
A cloud storage solution for storing large amounts of unstructured data, such as files and media.
``
``
``
``
...
``
``
A lightweight Python web framework used for building APIs and web applications.
``
``
``
``
...
``
``
A digital credential used to verify a user's identity and authorize access to APIs or services.
``
``
``
``
...
``
``
The number of seconds that have elapsed since January 1, 1970 (UTC), commonly used in computing for time representation.
``
``
``
``
...
| POST-Download File | This API is used for downloading files from the storage blob. |

These APIs are described further in the subsequent sections.
...
| Protocol | HTTPS |
|----|----|
| PATH AOT (Cognite)https://apim-mw-aot-dev.azure-api.net/api/filehandler/files/upload](https://apim-mw-aot-dev.azure-api.net/api/filehandler/files/upload) |
| PATH AOT (Azure)https://apim-aot-azure-dev.azure-api.net/api/filehandler/files/upload](https://apim-aot-azure-dev.azure-api.net/api/filehandler/files/upload) |
| METHOD | POST |
...
| File | Provide the file that needs to be uploaded | M | File |
...
``
Bad request
Please select the proper file extension
Storage account not configured for a given base.
...
``
Bad request
Invalid file path. Please reach out to Admin for a resolution
Storage account not configured for the given base: '`{base_name}`'. Please reach out to Admin for resolution.
``
``
``
``
