---
id: aot-people-management-architecture-blueprint-auriga
...
``
``
People Management Configuration UI
``
``
This layer is the interface provided to admin users. Using this interface, admin users can create and manage AOT Roles and Departments. Additionally, this layer can be used to assign AOT roles to various Departments, Active Directory groups, and users.
``
``
``
``
People Management APIs and Backend Services
``
``
This layer enables querying and updating User and Role information, mapping the Roles to Active Directory Groups, and checking user authorizations. All AOT backend services use People Management services/APIs to determine the exact data access the AOT user has and limit the returned datasets only to the data accessible by the user, based on their role.
``
``
``
``
CDF DataOps platform
``
``
The platform where the data is stored and tagged (in CDF resource metadata) with all the relevant information, enabling the user to access the data and also determines the level of access (read/write)
``
``
``
``
...
``
``
Admin roles have permission to access the Twin Builder component.
``
``
Based on the role assigned, the user can view KPIs, Insights, and Actions data on Twin Viewer.
...
``
PM Admin Component Responsibilities
End User View
``
``
...
``
``
Admin assigns level (s) in AH and the role they belong to.
Admin should have the flexibility to define if the role has access to sensitive data (KPI).
The PM engine should auto-assign owner and viewer access based on the KPI hierarchy (Contributing and Influencing KPIs).
A sensitivity tag is enabled in the Smart KPI configuration for sensitive KPIs and the Roles that could access it.
``
``
Users will have access to all the underlying levels in the AH they belong to.
Users cannot see information beyond the AH level they belong to.
In the landing screen of the Smart KPIs, the user will see the KPIs for whom they are an owner.
...
``
``
Assigns owner of the Insight in IA configuration screen.
Relates Insight with KPI(s).
Leverages KPI configuration (to the maximum extent possible) for Insight configuration for KPI-based Insights.
The PM engine should auto-assign Owner and viewer access based on Insight configuration (Refer to items on the right side)
``
``
Users will have access to all the underlying levels in the AH they belong to.
Users cannot see information beyond the AH level they belong to.
Users will see the list of Insights that they are responsible for (assigned to) – in the capacity of Insight owner.
Through the KPIs that the user has access to, they can view the insights related to those KPIs, even if these insights in question have not been directly assigned to the user.
